...
var ListViewStaggeredLayout = (function (_super) {
    ...
    //    private _localDelegate;
    ListViewStaggeredLayout.prototype.getNativeLayout = 
    //    private _localDelegate;
    function () {
...
var 
// ExtendedListViewCell
ExtendedListViewCell = (function (_super) {
    ...
    // This shows in the profiling. Can it be avoided?
    ExtendedListViewCell.prototype.touchesBeganWithEvent = 
    // This shows in the profiling. Can it be avoided?
    ...
    // This shows in the profiling. Can it be avoided?
    // This shows in the profiling. Can it be avoided?
    ExtendedListViewCell.prototype.touchesMovedWithEvent = 
    // This shows in the profiling. Can it be avoided?
    function (touches, event) {
...
var 
// ListView
RadListView = (function (_super) {
    __extends(RadListView, _super);
    function RadListView() {
        var _this = _super.call(this) || this;
        _this.defaultGroupTemplate = "<Label text='{{ category }}' height='50' backgroundColor='white' paddingLeft='15'></Label>";
    ...
    RadListView.prototype.reloadDataSource = function () {
        this._dataSource = TKListViewDataSourceImpl.initWithOwner(new WeakRef(this)); //weak ref
    ...
    RadListView.prototype.onGroupingFunctionChanged = function (oldValue, newValue) {
        if (this.groupingFunction) {
            if (this.headerItemTemplate) {
                console.log("Warning: Setting custom 'headerItemTemplate' with 'groupingFunction' enabled is not supported on iOS.");
            }
            this.headerItemTemplate = this.defaultGroupTemplate;
        }
        else {
            this.headerItemTemplate = undefined;
        }
        this.refresh();
    };
    ...
    RadListView.prototype.getDataItemFromSection = function (index, section, isGroup) {
        ...
        var value;
        if (isGroup) {
            value = { category: this.groupingFunction(dataSourceImp.nativeTKDataSource.items[section].items[index]), itemName: dataItem.itemName };
        }
        else {
            value = dataItem;
        }
        var groupObj = value;
        return this._dataSource && dataSourceImp.nativeTKDataSource.items.count > 0 && dataSourceImp.nativeTKDataSource.items[section].items ? groupObj : this.getDataItem(index);
    };
